...
describe('task generalization',()=>{
 ...
 it.each([demoTasks[0],demoTasks[1],demoTasks[4]])('generates task-aware unverified claims for $title',async task=>{const result=await (await handleChat(request({...payload,task,stage:'test',message:'My measured results need evidence across all conditions.',history:[{role:'student',text:'I observed different results in repeated trials.'},{role:'student',text:'I measured each result because repeated observations show variation.'}],intent:'challenge',mode:'demo'}))).json();expect(result.text).toContain(task.successCriteria![0]);const feedback=await (await handleChat(request({...payload,task,intent:'evaluate-claim',claim:result.text,mode:'demo'}))).json();expect(feedback.text).toContain('证据');expect(feedback.text).not.toMatch(/正确|错误/)});
